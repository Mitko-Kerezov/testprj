...
class ReversePlatformSpecificPathsPlugin {
  apply(compiler) {
    compiler.hooks.environment.tap('ReversePlatformSpecificPathsPlugin', () => {
      const originalPurge = compiler.inputFileSystem.purge.bind(compiler.inputFileSystem);
      compiler.inputFileSystem.purge = changes => {
        let newChanges = this.removePlatformSpecific(changes);

        originalPurge(newChanges);
        originalPurge(changes);
      }
    });
  }

  removePlatformSpecific(changes) {
    if (typeof changes === 'string') {
      return this.replacePlatformSpecificSuffix(changes);
    }

    if (Array.isArray(changes)) {
      let newChanges = [];
      changes.forEach((fileName) => {
        newChanges.push(this.replacePlatformSpecificSuffix(fileName));
      });
  ...
  replacePlatformSpecificSuffix(fileName) {
    return fileName.replace(/\.tns|\.ios|\.android/g, "");
  }
}
...
module.exports = env => {
  ...
  const config = {
    ...
    plugins: [
      ...
      ngCompilerPlugin,
      new ReversePlatformSpecificPathsPlugin(),
